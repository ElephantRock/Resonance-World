#!/usr/bin/env python3
# ruff: noqa: E501
"""Aggregate two Q3-D3 provider shards into one canonical diagnostic payload."""
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="output/d2-vnext-q3d3-canonical")
    args = parser.parse_args()
    root = Path(args.input_dir)
    paths = sorted(root.rglob("d2-vnext-q3d3-provider-shard-??.json"))
    defects: list[str] = []
    ...
    canonical = {
        "schema": "d2-vnext-q3d3-canonical-provider-output-v0.1",
        ...
        "shard_summaries": [
            {
                ...
                "http_body_records_observed": row.get("transport_accounting", {}).get(
                    "http_body_records_observed"
                ),
            }
    ...
    path = out / "d2-vnext-q3d3-provider-output.json"
    path.write_bytes(canonical_bytes(canonical))
    manifest = {
        "schema": "d2-vnext-q3d3-canonical-provider-manifest-v0.1",
