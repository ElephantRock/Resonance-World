#!/usr/bin/env python3
"""Deterministically aggregate D2b provider shards without classifying them."""
...
def load_shard_map(path: Path) -> dict[int, tuple[int, int]]:
    ...
    for row in rows:
        ...
        if shard_id in result:
            raise AssertionError("duplicate D2b shard id in map")
        result[shard_id] = (start, end)
        union.extend(range(start, end + 1))
    if sorted(result) != list(range(SHARD_COUNT)):
        raise AssertionError("D2b shard ids must be 0..17")
    if union != list(range(PAIR_COUNT)):
        raise AssertionError("D2b shard map must cover 0..359 exactly once")
    return result
...
def _validate_shard(
    ...
    *,
    ...
    defects: list[str] = []
    if data.get("schema") != "d2b-replication-provider-shard-v0.1":
...
def aggregate(
    ...
    *,
    ...
    for path in sorted(input_dir.rglob("d2b-replication-provider-shard-*.json")):
        try:
            data = json.loads(path.read_text())
        except Exception:
    ...
    for shard_id in range(SHARD_COUNT):
        ...
        if shard_defects:
            defects.extend(f"shard_{shard_id}:{defect}" for defect in shard_defects)
            invalid_shards.append(shard_id)
            pair_records.extend(
                _placeholder(index, shard_id, "invalid_shard_artifact")
                for index in range(start_pair, end_pair + 1)
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="output/d2b-replication-aggregated")
    ...
    output_path = output_dir / "d2b-replication-provider-output.json"
    output_path.write_bytes(canonical_bytes(output))
    manifest = {
        "schema": "d2b-replication-aggregation-manifest-v0.1",
    ...
    (output_dir / "aggregation-manifest.json").write_bytes(canonical_bytes(manifest))
