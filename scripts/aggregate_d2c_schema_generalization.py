...
def load_shard_map(path: Path) -> dict[int, tuple[int, int, str]]:
    ...
    if (
        data.get("pair_count_attempted") != PAIR_COUNT
    ...
    for row in rows:
        ...
        if (
            core.schema_and_local_index(start)[0] != schema_id
            or core.schema_and_local_index(end)[0] != schema_id
        ):
...
def _validate_shard(
    ...
    *,
    ...
    defects: list[str] = []
    if data.get("schema") != "d2c-schema-generalization-provider-shard-v0.1":
    ...
    if cohort.get("all_source_destination_overlaps_zero") is not True:
        defects.append("source_destination_overlap")
    if cohort.get("all_development_evaluation_overlaps_zero") is not True:
        defects.append("development_evaluation_overlap")
    ...
    if not isinstance(records, list) or len(records) != expected_n:
        ...
        if [row.get("pair_index") for row in records] != expected_indices:
            defects.append("pair_record_identity")
        if any(row.get("schema_id") != schema_id for row in records):
            defects.append("pair_record_schema")
    ...
    *,
    ...
    for shard_id in range(SHARD_COUNT):
        ...
        if not rows:
            ...
            pair_records.extend(
                _placeholder(i, shard_id, "missing_shard_artifact") for i in range(start, end + 1)
            )
            continue
        if len(rows) != 1:
            ...
            pair_records.extend(
                _placeholder(i, shard_id, "invalid_shard_artifact") for i in range(start, end + 1)
            )
            continue
        _path, data = rows[0]
    ...
    schema_counts = {
        schema_id: {
            ...
            "complete": sum(
                row.get("schema_id") == schema_id and row.get("status") == "complete"
