...
def preflight() -> dict[str, Any]:
    ...
    if contract.sha(prompt) != contract.SYSTEM_PROMPT_SHA256:
        raise AssertionError("canonical system prompt drift")
    required_fragments = (
        ...
        'Choose actions for the actual presented cases',
    )
    if not all(fragment in prompt for fragment in required_fragments):
        raise AssertionError("canonical prompt contract incomplete")
    ...
    return {
        "schema": "d2-canonical-json-prompt-preflight-v0.1",
