...
assert p["canonical_exemplar_copy_disqualifies_pass"] is True
assert p["prompt_contract_failure_outcome"]=="FAIL_PROMPT_CONTRACT"
...
assert 0<=p["canonical_exemplar_copy_count"]<=72
assert 0<=p["physical_provider_sends_observed_total"]<=180
assert outcome in {"PASS","FAIL_PROMPT_CONTRACT","FAIL_STRUCTURED_CONTRACT","FAIL_JSON_MODE_COMPATIBILITY","FAIL_COMPLETION","APPARATUS_FAILURE"}
...
if outcome=="PASS":
    ...
    assert p["canonical_exemplar_copy_count"]==0
    ...
    assert all(r["final_response_length"]>0 and r["exact_structured_parse_valid"] and not r["canonical_exemplar_copy"] and r["exact_attributed_clean_transport"] for r in p["probes"])
if outcome=="FAIL_PROMPT_CONTRACT":
    assert p["canonical_exemplar_copy_count"]>0
    assert p["exact_structured_parse_invalid_count"]==0
    assert p["json_mode_compatibility_failure_count"]==0
    assert p["apparatus_failure"] is False
