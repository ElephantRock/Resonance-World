"""Pure frozen statistics for D2 confirmatory evaluation."""
...
def evaluate_scores(
    ...
    """Apply the frozen D2 serial gatekeeping contract."""
    n = len(source)
    if not (n == len(fresh) == len(description) == len(reproduced)):
        raise ValueError("arm length mismatch")
    if n < MIN_ANALYZABLE:
    ...
    p1_values = [
        r - d for r, d in zip(reproduced, description, strict=True)
    ]
