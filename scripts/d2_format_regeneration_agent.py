# ruff: noqa: I001,E501
"""Pinned Hermes request construction and zero-provider preflight for #276."""
...
def preflight() -> dict[str, Any]:
    ...
    if not all(fragment in prompt for fragment in required_fragments):
        raise AssertionError("primary JSON-skeleton prompt contract incomplete")
    ...
    retry = contract.retry_user_prompt(sample_probe, "json_decode_failure")
    if "Raw prior response content is not provided" not in retry or "json_decode_failure" not in retry:
        raise AssertionError("retry prompt contract incomplete")
    if forbidden_exemplar in retry:
        raise AssertionError("retry literal valid action exemplar prohibited")
    ...
    retry_lengths = [len(contract.retry_user_prompt(probe, "json_decode_failure").encode()) for probe in probes]
    return {
        "schema": "d2-format-regeneration-preflight-v0.1",
        "issue": contract.ISSUE,
        "provider_execution_performed": False,
        "execution_marker_absent": True,
        "fresh_namespace": contract.NAMESPACE,
        "probe_count": len(probes),
        "minimum_primary_prompt_bytes": min(lengths),
        "maximum_primary_prompt_bytes": max(lengths),
        "minimum_retry_prompt_bytes": min(retry_lengths),
        "maximum_retry_prompt_bytes": max(retry_lengths),
