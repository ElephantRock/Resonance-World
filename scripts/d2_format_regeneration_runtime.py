...
from d2_format_regeneration_agent import new_agent, preflight, request_overrides
from d2_format_regeneration_probe import apparatus_failure, retry_eligible, run_probe
...
def execute() -> dict[str, Any]:
    ...
    with budget.propagate_to_child_threads():
        with transport.transport_guard(budget, ledger, workers):
            with ThreadPoolExecutor(max_workers=contract.MAX_CONCURRENCY) as pool:
                rows = list(pool.map(lambda p: run_probe(budget, ledger, p), probes))

    attempted = len(rows)
    effective = sum(bool(row["effective_completed"]) for row in rows)
    native = sum(bool(row["effective_completed"]) and bool(row["hermes_completed"]) for row in rows)
    overrides = sum(bool(row["terminal_iteration_override_used"]) for row in rows)
    first_invalid = sum(
        bool(row["first_attempt"]["final_response_length"])
        and not bool(row["first_attempt"]["exact_structured_parse_valid"])
        for row in rows
    )
    retry_used = sum(bool(row["retry_used"]) for row in rows)
    retry_success = sum(row["accepted_attempt_index"] == 2 for row in rows)
    exact_invalid = sum(
        bool(row["final_response_length"]) and not bool(row["exact_structured_parse_valid"])
        for row in rows
    )
    placeholder_leaks = sum(bool(row["placeholder_leak"]) for row in rows)
    ...
    base_pass = (
        ...
        and all(row["accepted_attempt_index"] in {1, 2} for row in rows)
        ...
        and all(not row["retry_policy_violation"] for row in rows)
        and compatibility == 0
        and not apparatus
    )
    ...
    return {
        "schema": "d2-format-regeneration-result-v0.1",
        ...
        "retry_raw_first_response_content_allowed": False,
        ...
        "hermes_pyproject_blob_sha": contract.HERMES_PYPROJECT_BLOB_SHA,
        "openai_sdk_version": contract.OPENAI_VERSION,
        "openai_sdk_default_max_retries": retries,
        "httpx_version": httpx_version,
        "max_iterations_per_agent_invocation": contract.MAX_ITERATIONS,
        "maximum_agent_invocations_per_probe": contract.MAX_AGENT_INVOCATIONS,
        ...
        "first_attempt_exact_structured_parse_invalid_count": first_invalid,
        "retry_used_count": retry_used,
        "retry_success_count": retry_success,
        "terminal_exact_structured_parse_invalid_count": exact_invalid,
        "placeholder_leak_count": placeholder_leaks,
