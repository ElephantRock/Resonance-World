# ruff: noqa: I001,E501
"""Fail-closed transport instrumentation for #276 format-regeneration qualification."""
...
class TransportLedger:
    ...
    def begin(self, reservation: SendReservation, origin_logical_index: int) -> int:
        if origin_logical_index != reservation.logical_index:
            ...
            raise LogicalAttributionMismatch("request origin disagrees with provider-send reservation")
        row = {
...
def verified_origin_logical_index(request: Any, budget: ProviderSendBudget, ledger: TransportLedger) -> int:
    ...
    if not 0 <= origin < contract.MAX_LOGICAL_CALLS or registered is None or origin != registered:
        ledger.record_attribution_mismatch()
        raise LogicalAttributionMismatch("independent request origin does not match registered logical context")
    return origin
...
class ProviderWorkerTracker:
    ...
    def drain(self, timeout_seconds: float = contract.PROVIDER_WORKER_DRAIN_TIMEOUT_SECONDS) -> int:
...
def transport_guard(
    ...
    """Guard physical sends; restore HTTP hooks only after Hermes workers drain."""
    import httpx
    original_sync = httpx.Client._send_single_request
    original_async = httpx.AsyncClient._send_single_request
    original_thread_start = threading.Thread.start

    def tracked_thread_start(thread: threading.Thread, *args: object, **kwargs: object) -> object:
