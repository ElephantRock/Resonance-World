"""Per-probe execution for #263 top-level projection conformance."""
...
from resonance_world import d2_top_level_projection as projection
...
def _view(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "final_response": (
            "bounded-nonempty-response-sentinel" if row["final_response_length"] else ""
        ),
    }
...
def _decision(
    ...
    return adapter.evaluate_terminal_completion(
        ...
        parse_valid=bool(row["projected_structured_parse_valid"]),
...
def run_probe(
    ...
    try:
        ...
        exact_valid, _ = adapter.parse_response(final_text)
        projected = projection.parse_projected_response(final_text)
        if projected.exact_parse_valid != exact_valid:
            raise AssertionError("exact/projected parser consistency drift")
        ...
        row.update(
            {
                ...
                "projected_structured_parse_valid": projected.valid,
                "structured_parse_valid": projected.valid,
                "parse_diagnostic": projected.diagnostic,
                "projection_used": projected.projection_used,
                "unknown_top_level_key_count": projected.unknown_key_count,
                "unknown_top_level_key_fingerprints": list(
                    projected.unknown_key_fingerprints
                ),
                "strategy_length": len(projected.strategy) if projected.valid else 0,
                "strategy_sha256": (
                    contract.sha(projected.strategy)
                    if projected.valid and projected.strategy
                    else None
                ),
            }
        )
