"""One-shot execution aggregation for #263 top-level projection conformance."""
...
def execute() -> dict[str, Any]:
    ...
    projected_invalid = sum(
        bool(row["final_response_length"])
        and not bool(row["projected_structured_parse_valid"])
        for row in rows
    )
    projections = sum(bool(row["projection_used"]) for row in rows)
    projected_unknown_keys = sum(int(row["unknown_top_level_key_count"]) for row in rows)
    ...
    base_pass = (
        ...
        and all(row["projected_structured_parse_valid"] for row in rows)
    ...
    elif base_pass and projections == 0:
    ...
    return {
        "schema": "d2-top-level-projection-result-v0.1",
        ...
        "projected_structured_parse_invalid_count": projected_invalid,
        "projection_used_probe_count": projections,
        "projected_unknown_top_level_key_count": projected_unknown_keys,
