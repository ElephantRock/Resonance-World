# ruff: noqa
"""Frozen constants and validation for #273 JSON-skeleton prompt conformance."""
...
AUTH_ENV = "D2_JSON_SKELETON_AUTHORIZED"
...
PLAN_GIT_BLOB_SHA = "c5e7681a2b177e4a1df2aefba7eea770bb52a225"
PROBES_GIT_BLOB_SHA = "237468365dee86076999738f9a418890a0a912b6"
...
def load_probes() -> list[dict[str, Any]]:
    ...
    expected = {
        "schema": "d2-json-skeleton-probes-v0.1",
        "issue": ISSUE,
        "namespace": NAMESPACE,
        "seed_start": 4400001,
    ...
    for shape in payload["shape_order"]:
        for local in range(18):
            ...
            rows.append({
                ...
                "probe_id": f"skeleton_{shape}_{local:02d}",
                "call_shape": shape,
...
def validate_frozen_contract() -> list[dict[str, Any]]:
    ...
    required = {
        "schema": "d2-json-skeleton-request-plan-v0.1",
        "issue": ISSUE,
    ...
    for key in (
        ...
        "non_answer_positional_skeleton_required",
        "no_literal_valid_action_exemplar_required",
        "placeholder_leak_bounded_diagnostic",
    ...
    if min(row["seed"] for row in rows) <= 4_400_000:
