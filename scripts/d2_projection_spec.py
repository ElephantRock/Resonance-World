...
DIR = ROOT / "research" / "d2_top_level_projection"
PLAN = DIR / "REQUEST_PLAN.json"
PROBES = DIR / "PROBES.json"
MARKER = DIR / "RUN_D2_TOP_LEVEL_PROJECTION"
GUARD_PATH = ROOT / "src" / "resonance_world" / "provider_send_guard.py"
ADAPTER_PATH = ROOT / "src" / "resonance_world" / "d2_terminal_adapter.py"
PROJECTION_PATH = ROOT / "src" / "resonance_world" / "d2_top_level_projection.py"

ISSUE = 263
PR_NUMBER = 266
NAMESPACE = "rw.d2-top-level-projection.v1"
...
AUTH_ENV = "D2_TOP_LEVEL_PROJECTION_AUTHORIZED"
...
PROJECTION_GIT_BLOB_SHA = "84ee0c1624f35ff0b8c68aad3721e48d134dea9e"
PLAN_GIT_BLOB_SHA = "cf1491572c3910a7a73e2f9f8427ae9e995ec0d5"
...
def load_probes() -> list[dict[str, Any]]:
    ...
    expected = {
        "schema": "d2-top-level-projection-probes-v0.1",
        "issue": ISSUE,
        "namespace": NAMESPACE,
        "seed_start": 4200000,
    ...
    for shape in payload["shape_order"]:
        for local in range(18):
            ...
            rows.append({
                ...
                "probe_id": f"projection_{shape}_{local:02d}",
                "call_shape": shape,
...
def validate_frozen_contract() -> list[dict[str, Any]]:
    for path, expected in (
        ...
        (PROJECTION_PATH, PROJECTION_GIT_BLOB_SHA),
    ):
        if git_blob_sha(path) != expected:
            raise AssertionError(f"blob drift: {path.name}")
    plan = json.loads(PLAN.read_text())
    required = {
        "schema": "d2-top-level-projection-request-plan-v0.1",
        "issue": ISSUE,
    ...
    if len({row["probe_id"] for row in rows}) != 72 or len({row["seed"] for row in rows}) != 72 or min(row["seed"] for row in rows) < 4_100_000:
