"""Fresh deterministic namespace for D2-vNext-Q1 acquisition qualification."""
...
NAMESPACE = "rw.d2-vnext-q1-acquisition-qualification.v1"
STAGE_A = "Q1-A"
STAGE_B = "Q1-B"
...
STAGE_SCHEMA_SEED_BASES = {
    STAGE_A: {
        "threshold_at_4": 11_000_000,
        "parity_pair": 11_200_000,
        "interval_pair": 11_400_000,
        "pairwise_order": 11_600_000,
    },
    STAGE_B: {
        "threshold_at_4": 12_000_000,
        "parity_pair": 12_200_000,
        "interval_pair": 12_400_000,
        "pairwise_order": 12_600_000,
    },
}
...
def pairs_per_schema(stage: str) -> int:
    try:
        ...
        raise ValueError(f"unknown D2-vNext-Q1 stage: {stage}") from exc
...
def schema_and_local_index(stage: str, global_pair_index: int) -> tuple[str, int]:
    ...
    if not 0 <= global_pair_index < count:
        raise ValueError("global D2-vNext-Q1 pair index out of range")
...
def pair_seed_for(stage: str, schema_id: str, local_pair_index: int) -> int:
    ...
    if schema_id not in bases:
        raise ValueError(f"unknown D2-vNext-Q1 schema: {schema_id}")
    if not 0 <= local_pair_index < pairs_per_schema(stage):
        raise ValueError("local D2-vNext-Q1 pair index out of range")
