"""Fresh deterministic pairwise-only namespace for D2-vNext-Q3-D diagnostics."""
...
NAMESPACE = "rw.d2-vnext-q3d-terminal-boundary-diagnostic.v1"
STAGE = "Q3-D"
SCHEMA_ID = "pairwise_order"
PAIR_COUNT = 16
PAIR_SEED_BASE = 15_000_000
...
def schema_and_local_index(global_pair_index: int) -> tuple[str, int]:
    if not 0 <= global_pair_index < PAIR_COUNT:
        raise ValueError("global D2-vNext-Q3-D pair index out of range")
    return SCHEMA_ID, global_pair_index


def pair_seed_for(local_pair_index: int) -> int:
    if not 0 <= local_pair_index < PAIR_COUNT:
        raise ValueError("local D2-vNext-Q3-D pair index out of range")
