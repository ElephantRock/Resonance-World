"""Fresh deterministic namespace for D2-vNext-S2 acquisition calibration."""
...
NAMESPACE = "rw.d2-vnext-s2-source-acquisition.v1"
...
def schema_and_local_index(global_pair_index: int) -> tuple[str, int]:
    ...
    if not 0 <= global_pair_index < pair_count:
        raise ValueError("global D2-vNext-S2 pair index out of range")
...
def pair_seed_for(schema_id: str, local_pair_index: int) -> int:
    if schema_id not in SCHEMA_SEED_BASES:
        raise ValueError(f"unknown D2-vNext-S2 schema: {schema_id}")
    if not 0 <= local_pair_index < PAIRS_PER_SCHEMA:
        raise ValueError("local D2-vNext-S2 pair index out of range")
