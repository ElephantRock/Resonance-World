#!/usr/bin/env python3
"""Deterministically evaluate canonical D2b replication provider output."""
...
import d2_c2_confirmatory_stats as stats
import run_d2b_replication as runner
...
def _global_integrity(
    ...
    *,
    ...
    defects: list[str] = []
    if provider.get("schema") != "d2b-replication-provider-output-v0.1":
    ...
    aggregation = provider.get("aggregation_integrity", {})
...
def evaluate(
    ...
    *,
    ...
    if integrity_ok:
        ...
        statistical_result = stats.evaluate_scores(fresh, description, reproduced, source)
    ...
    return {
        "schema": "d2b-replication-result-v0.1",
        ...
        "replication_classification_map": {
            "D2-S0": "D2b-S0",
            "D2-S1": "D2b-S1",
            "D2-S2": "D2b-S2",
            "D2-S3": "D2b-S3",
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="output/d2b-replication-evaluation")
    parser.add_argument("--plan", default="research/d2b/PLAN.md")
    parser.add_argument(
        "--request-plan", default="research/d2b/D2B_REPLICATION_REQUEST_PLAN.json"
    )
    parser.add_argument(
        "--cohort-lock", default="research/d2b/d2b-replication-cohort-lock.json"
    )
    ...
    result_path = output_dir / "d2b-replication-result.json"
    result_path.write_bytes(canonical_bytes(result))
    manifest = {
        "schema": "d2b-replication-evaluation-manifest-v0.1",
        "provider_output_sha256": result["provider_output_sha256"],
        "result_sha256": file_sha256(result_path),
        "classification": result["classification"],
        "d2b_classification": result["d2b_classification"],
        "analyzable_pairs": result["analyzable_pairs"],
        "registry_promotion_authorized": False,
        "production_historical_substrate_enabled": False,
    }
    (output_dir / "evaluation-manifest.json").write_bytes(canonical_bytes(manifest))
