...
def _global_integrity(
    ...
    *,
    ...
    schema_suite_path: Path,
    sample_size_path: Path,
    cohort_lock_path: Path,
    shard_map_path: Path,
) -> list[str]:
    defects: list[str] = []
    if provider.get("schema") != "d2c-schema-generalization-provider-output-v0.1":
    ...
    if provider.get("classification") is not None:
        defects.append("provider_must_not_classify")
    ...
    if float(provider.get("temperature", -1)) != EXPECTED_TEMPERATURE:
    ...
    if provider.get("aggregation_integrity", {}).get("passed") is not True:
        defects.append("aggregation_integrity")
    cohort = provider.get("cohort_lock", {})
    if cohort.get("pair_count") != EXPECTED_ATTEMPTED_PAIRS:
        defects.append("cohort_pair_count")
    if cohort.get("pairs_per_schema") != 180:
        defects.append("cohort_pairs_per_schema")
...
def _pair_integrity(row: dict[str, Any]) -> tuple[list[str], dict[str, float] | None]:
    ...
    if row.get("schema_id") != expected_schema:
        defects.append("schema_id")
    if row.get("schema_pair_index") != expected_local:
        defects.append("schema_pair_index")
    bundle = runner.case_bundle(pair_index)
    if row.get("pair_lock_record") != runner.pair_lock_record(pair_index):
    ...
    for arm, expected_calls_n in expected_calls.items():
        ...
        if not isinstance(record, dict):
            ...
            continue
        if record.get("logical_calls") != expected_calls_n:
...
def _classify(schema_results: dict[str, dict[str, Any]], integrity_ok: bool) -> tuple[str, str]:
    ...
    if any(not result["P1"]["gate_pass"] for result in schema_results.values()):
        return (
            "D2c-S2",
            "schema_generalization_reproduction_beyond_description_not_established_all_schemas",
        )
...
def evaluate(
    ...
    *,
    ...
    global_defects = _global_integrity(
        ...
        schema_suite_path=schema_suite_path,
    ...
    analyzable: dict[str, list[tuple[int, dict[str, float]]]] = {
    ...
    for schema_offset, schema_id in enumerate(core.SCHEMA_ORDER):
        ...
        schema_results[schema_id] = (
            stats.evaluate_schema_scores(
                fresh, description, reproduced, source, bootstrap_seed_offset=schema_offset
            )
            if integrity_ok
            else {
    ...
    return {
        ...
        "analyzable_pairs_by_schema": {
            schema_id: len(analyzable[schema_id]) for schema_id in core.SCHEMA_ORDER
        },
        ...
        "program_decision_rule": (
            "all_three_schemas_must_independently_pass_minimum_n_P0_P1_P2_no_pooling_or_rescue"
        ),
...
def main() -> None:
    ...
    result = evaluate(
        ...
        schema_suite_path=Path(args.schema_suite),
        sample_size_path=Path(args.sample_size),
