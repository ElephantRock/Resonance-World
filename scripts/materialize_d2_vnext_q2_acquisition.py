#!/usr/bin/env python3
# ruff: noqa: E501
"""Deterministically materialize D2-vNext-Q2 Q2-A/Q2-B qualification cohorts."""
...
EXPECTED_COHORT_SHA256: dict[str, str] = {
    core.STAGE_A: "74ac69fc045c88e93ea08e86d031dcae23fa179bcba03d2b170526085e00b61b",
    core.STAGE_B: "7ccace45abe7df9eb510c72649d99650b7a5b26d66829a65c638d762ce765e53",
}
...
PREDECESSOR_NAMESPACES = {
    ...
    "D2-vNext-Q1": [(11_000_000, 11_699_999), (12_000_000, 12_699_999)],
}
...
def case_bundle(stage: str, pair_index: int) -> dict[str, Any]:
    ...
    if development_features & core.features_set(evaluation_cases):
        raise AssertionError("D2-vNext-Q2 development/evaluation feature overlap")
    return {
        "stage": stage,
...
def build_cohort_lock(stage: str) -> dict[str, Any]:
    ...
    if len(seeds) != len(set(seeds)):
        raise AssertionError(f"D2-vNext-Q2 {stage} seed collision")
    ...
    if any(predecessor.values()):
        raise AssertionError(f"D2-vNext-Q2 predecessor seed overlap: {predecessor}")
    cohort_hash = core.sha256(records)
    expected = EXPECTED_COHORT_SHA256[stage]
    if cohort_hash != expected:
        raise AssertionError(f"D2-vNext-Q2 {stage} cohort drift: {cohort_hash}")
    return {
        "schema": "d2-vnext-q2-acquisition-cohort-lock-v0.1",
        "study_stream": "D2-vNext-Q2",
        "stage": stage,
...
def build_shard_map(stage: str) -> dict[str, Any]:
    ...
    if pair_count % PAIRS_PER_SHARD:
        raise AssertionError("Q2 pair count must divide into 16-pair shards")
    ...
    for shard_id in range(shard_count):
        ...
        if any(count != PAIRS_PER_SCHEMA_PER_SHARD for count in schema_counts.values()):
            raise AssertionError("Q2 mixed-schema shard balance drift")
    ...
    if coverage != list(range(pair_count)):
        raise AssertionError("Q2 shard coverage mismatch")
    return {
        "schema": "d2-vnext-q2-acquisition-shard-map-v0.1",
        "study_stream": "D2-vNext-Q2",
        "stage": stage,
        "fresh_namespace": core.NAMESPACE,
...
def validate_cross_stage_seed_separation() -> None:
    ...
    if stage_seeds[core.STAGE_A] & stage_seeds[core.STAGE_B]:
        raise AssertionError("Q2-A/Q2-B seed overlap")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--output-dir", default="output/d2-vnext-q2-materialization")
    args = parser.parse_args()
    validate_cross_stage_seed_separation()
    out = Path(args.output_dir)
    out.mkdir(parents=True, exist_ok=True)
    outputs: dict[str, dict[str, Any]] = {}
    for stage, slug in ((core.STAGE_A, "Q2_A"), (core.STAGE_B, "Q2_B")):
        outputs[f"D2_VNEXT_{slug}_COHORT_LOCK.json"] = build_cohort_lock(stage)
        outputs[f"D2_VNEXT_{slug}_SHARD_MAP.json"] = build_shard_map(stage)
    for name, payload in outputs.items():
        (out / name).write_bytes(canonical_bytes(payload))
    manifest = {
        "schema": "d2-vnext-q2-acquisition-materialization-manifest-v0.1",
        "study_stream": "D2-vNext-Q2",
        "fresh_namespace": core.NAMESPACE,
        "q2_a_cohort_pairs_sha256": outputs["D2_VNEXT_Q2_A_COHORT_LOCK.json"]["cohort_pairs_sha256"],
        "q2_b_cohort_pairs_sha256": outputs["D2_VNEXT_Q2_B_COHORT_LOCK.json"]["cohort_pairs_sha256"],
