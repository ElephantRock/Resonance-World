#!/usr/bin/env python3
# ruff: noqa: E501
"""Deterministically materialize the fresh Q3-D2 request-scoped diagnostic cohort."""
...
EXPECTED_COHORT_SHA256 = "fb907354a76cbe843a7f1a59cacffdde2fb7a5d64afe1f7be2585e66ce4b0321"
...
def case_bundle(pair_index: int) -> dict[str, Any]:
    ...
    development_cases = core.generate_balanced_cases(
        ...
        prefix=f"d2-vnext-q3d2-{schema_id}-p{local_index:03d}-dev",
        policy=policy,
    )
    ...
    evaluation_cases = core.generate_balanced_cases(
        ...
        prefix=f"d2-vnext-q3d2-{schema_id}-p{local_index:03d}-eval",
        policy=policy,
        exclude_features=development_features,
    )
    if development_features & core.features_set(evaluation_cases):
        raise AssertionError("D2-vNext-Q3-D2 development/evaluation feature overlap")
    return {
        "stage": core.STAGE,
...
def build_cohort_lock() -> dict[str, Any]:
    ...
    if len(seeds) != core.PAIR_COUNT * 3:
        raise AssertionError("D2-vNext-Q3-D2 seed collision")
    predecessor = _predecessor_overlap(seeds)
    if any(predecessor.values()):
        raise AssertionError(f"D2-vNext-Q3-D2 predecessor seed overlap: {predecessor}")
    cohort_hash = core.sha256(records)
    if cohort_hash != EXPECTED_COHORT_SHA256:
        raise AssertionError(f"D2-vNext-Q3-D2 cohort drift: {cohort_hash}")
    return {
        "schema": "d2-vnext-q3d2-diagnostic-cohort-lock-v0.1",
...
def build_shard_map() -> dict[str, Any]:
    ...
    if coverage != list(range(core.PAIR_COUNT)):
        raise AssertionError("D2-vNext-Q3-D2 shard coverage mismatch")
    return {
        "schema": "d2-vnext-q3d2-diagnostic-shard-map-v0.1",
        "study_stream": "D2-vNext-Q3-D2",
        "stage": core.STAGE,
        "fresh_namespace": core.NAMESPACE,
        ...
        "maximum_registered_logical_calls_per_shard": (
            PAIRS_PER_SHARD * LOGICAL_CALLS_PER_COMPLETE_PAIR
        ),
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="output/d2-vnext-q3d2-materialization")
    ...
    files = {
        "D2_VNEXT_Q3D2_COHORT_LOCK.json": cohort,
        "D2_VNEXT_Q3D2_SHARD_MAP.json": shard_map,
    }
    for name, payload in files.items():
        (out / name).write_bytes(canonical_bytes(payload))
    manifest = {
        "schema": "d2-vnext-q3d2-materialization-manifest-v0.1",
        "study_stream": "D2-vNext-Q3-D2",
        "fresh_namespace": core.NAMESPACE,
