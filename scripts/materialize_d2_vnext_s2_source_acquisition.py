#!/usr/bin/env python3
"""Deterministically materialize the D2-vNext-S2 cohort and shard topology."""
...
PREDECESSOR_NAMESPACES = {
    ...
    "D2-engineering-4.3m": [(4_300_000, 4_399_999)],
...
def case_bundle(pair_index: int) -> dict[str, Any]:
    ...
    development_cases = core.generate_balanced_cases(
        ...
        prefix=f"d2-vnext-s2-{schema_id}-p{local_index:03d}-dev",
        policy=policy,
    )
    ...
    evaluation_cases = core.generate_balanced_cases(
        ...
        prefix=f"d2-vnext-s2-{schema_id}-p{local_index:03d}-eval",
        policy=policy,
        exclude_features=development_features,
    )
    if development_features & core.features_set(evaluation_cases):
        raise AssertionError("D2-vNext-S2 development/evaluation feature overlap")
    return {
...
def build_cohort_lock() -> dict[str, Any]:
    ...
    if len(values) != len(set(values)):
        raise AssertionError("D2-vNext-S2 seed collision")
    ...
    if any(predecessor.values()):
        raise AssertionError(f"D2-vNext-S2 predecessor seed overlap: {predecessor}")
    cohort_hash = core.sha256(records)
    if EXPECTED_COHORT_SHA256 and cohort_hash != EXPECTED_COHORT_SHA256:
        raise AssertionError(f"D2-vNext-S2 cohort drift: {cohort_hash}")
    return {
        "schema": "d2-vnext-s2-source-acquisition-cohort-lock-v0.1",
...
def build_shard_map() -> dict[str, Any]:
    ...
    for shard_id in range(SHARD_COUNT):
        ...
        if schema_id != end_schema:
            raise AssertionError("D2-vNext-S2 provider shard crosses schema boundary")
    ...
    if coverage != list(range(PAIR_COUNT)):
        raise AssertionError("D2-vNext-S2 shard map coverage mismatch")
    return {
        "schema": "d2-vnext-s2-source-acquisition-shard-map-v0.1",
        "study_stream": "D2-vNext-S2",
        "fresh_namespace": core.NAMESPACE,
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="output/d2-vnext-s2-materialization")
    ...
    outputs = {
        "d2-vnext-s2-source-acquisition-cohort-lock.json": lock,
        "D2_VNEXT_S2_SHARD_MAP.json": shard_map,
    }
    for name, payload in outputs.items():
        (out / name).write_bytes(canonical_bytes(payload))
    manifest = {
        "schema": "d2-vnext-s2-source-acquisition-materialization-manifest-v0.1",
        "study_stream": "D2-vNext-S2",
        "fresh_namespace": core.NAMESPACE,
