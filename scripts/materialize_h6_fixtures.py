#!/usr/bin/env python3
"""Materialize preregistered H6 relevance-gating fixtures from the H5 g3 lineage."""
...
H5_SOURCE_CANDIDATE = "7afa2d139049b1fdb80de2a95d76b49430b6a046"
H5_PLANE_E_SHA256 = "173b3e8a6461c38eb3bad6e3dd7ed6b38807f512b5ff64bfc0ead4f19ed4cbb9"
H5_PLANE_K_SHA256 = "f59804690f06782133881d648b0bcd1cb94c818a6e2747d8d2754b0a45dddb19"
...
REPLICATES = tuple(f"r{i}" for i in range(1, 7))
ARMS = (
    "static_no_state",
    "persistent_no_state",
    "static_with_state",
    "persistent_with_state",
)
...
def action_records(
    *, family: str, unit_id: str, org: str, predicate: str, cutoff: int,
    actions: list[str], correct_position: int, provenance: list[str],
) -> list[dict[str, Any]]:
...
def procedure_records(
    *, family: str, unit_id: str, org: str, predicate: str, cutoff: int,
    correct_position: int, provenance: list[str],
) -> list[dict[str, Any]]:
    if family != "routine_transfer":
        return [
    ...
    return [
        record(record_id=f"{unit_id}-procedure-a", organization_id=org, predicate=predicate,
               ...
               payload={"status": "observed", "provenance_class": trusted, "attempts": 3, "successes": 3,
                        "episode_token": token("ep", unit_id, "a")}),
        record(record_id=f"{unit_id}-procedure-b", organization_id=org, predicate=predicate,
               observed_at=cutoff - 30, responsibility="steward", record_kind="procedure_outcome",
               payload={"status": "observed", "provenance_class": other, "attempts": 3, "successes": 0,
...
def authority_records(
    *, family: str, unit_id: str, org: str, predicate: str, cutoff: int,
    valid_position: int, capability: str,
) -> tuple[list[dict[str, Any]], dict[str, str]]:
    generation = "g3"
    notices = sorted([token("notice", unit_id, generation, 0, n=12), token("notice", unit_id, generation, 1, n=12)])
    valid_notice, invalid_notice = notices[valid_position], notices[1 - valid_position]
    valid_payload: dict[str, Any] = {
        "notice_id": valid_notice, "world_verification": True, "execution_capability": capability,
        "historical_label": "archival" if family == "authority_conflict" else "current",
    }
    invalid_payload: dict[str, Any] = {
        "notice_id": invalid_notice, "world_verification": False, "execution_capability": capability,
        "historical_label": "previously_authoritative" if family == "authority_conflict" else "inactive",
    }
    rows = [
        record(record_id=f"{unit_id}-{generation}-authority-valid", organization_id=org, predicate=predicate,
               observed_at=cutoff - 20, responsibility="steward", record_kind="authority_notice", payload=valid_payload),
        record(record_id=f"{unit_id}-{generation}-authority-stale", organization_id=org, predicate=predicate,
               observed_at=cutoff - 10, responsibility="steward", record_kind="authority_notice", payload=invalid_payload),
    ]
    rows.sort(key=lambda row: str(row["payload"]["notice_id"]))
    return rows, {"organization_id": org, "scenario_id": f"{unit_id}-g3", "action": capability, "notice_id": valid_notice}
...
def materialize() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    for idx in range(12):
        ...
        org, predicate = token("org", unit_id), token("pred", unit_id)
        ...
        members = {rep: {role: token("h6-member", unit_id, rep, role, n=14) for role in ROLES} for rep in REPLICATES}
        units.append({"unit_id": unit_id, "family": family, "organization_id": org, "predicate": predicate,
                      "actions": actions, "provenance_classes": provenance, "members": members, "record_budget": RECORD_BUDGET})
        evaluator_units.append({"unit_id": unit_id, "correct_action": actions[correct_position], "correct_position": correct_position})
        cutoff = 300_000 + idx * 100
        capability = f"execute:{unit_id}-g3"
        rows = action_records(family=family, unit_id=unit_id, org=org, predicate=predicate, cutoff=cutoff,
                              actions=actions, correct_position=correct_position, provenance=provenance)
        rows += procedure_records(family=family, unit_id=unit_id, org=org, predicate=predicate, cutoff=cutoff,
                                  correct_position=correct_position, provenance=provenance)
        authority_rows, grant = authority_records(family=family, unit_id=unit_id, org=org, predicate=predicate,
                                                   cutoff=cutoff, valid_position=(idx + 2) % 2, capability=capability)
        rows += authority_rows
        if len(rows) != RECORD_BUDGET:
            raise AssertionError("H6 requires exactly six H5-lineage g3 canonical records")
        evidence_sets[unit_id] = sorted(rows, key=lambda row: (int(row["observed_at"]), str(row["record_id"])))
        authority_grants.append(grant)
    lineage_bytes = cb({"canonical_evidence_sets": evidence_sets, "authority_grants": authority_grants,
                        "evaluator_units": evaluator_units})
    plane_e = {
        "schema": "h6-plane-e-v0.1", "world_preregistered_base": WORLD_BASE,
        "h5_source_candidate": H5_SOURCE_CANDIDATE, "h5_plane_e_sha256": H5_PLANE_E_SHA256,
        "h5_plane_k_sha256": H5_PLANE_K_SHA256, "h5_g3_lineage_sha256": hashlib.sha256(lineage_bytes).hexdigest(),
        "contextgraph_release_commit": CONTEXTGRAPH_COMMIT,
        "model_contract": {"provider": "zai-chat-completions", "model": MODEL, "endpoint": MODEL_ENDPOINT,
                           "do_sample": True, "temperature": 0.8, "thinking": {"type": "disabled"},
                           "stream": False, "response_format": {"type": "json_object"}, "max_output_tokens": 96},
        "generation": "g3", "replicates": list(REPLICATES), "factorial_arms": list(ARMS), "roles": list(ROLES),
        "record_budget": RECORD_BUDGET, "calls_per_cell": CALLS_PER_CELL, "units": units,
        "canonical_evidence_sets": evidence_sets, "authority_grants": authority_grants,
        "private_evaluator_sentinel": "ABSENT_FROM_PLANE_E_BY_CONSTRUCTION",
    }
    plane_k = {"schema": "h6-plane-k-v0.1", "private_evaluator_sentinel": PRIVATE_SENTINEL,
               "h5_source_candidate": H5_SOURCE_CANDIDATE, "units": evaluator_units}
    manifest = {
        "schema": "h6-fixture-manifest-v0.1", "unit_count": len(units),
        "organization_cell_count": len(units) * len(REPLICATES) * len(ARMS),
        "logical_model_call_count": len(units) * len(REPLICATES) * len(ARMS) * CALLS_PER_CELL,
        "canonical_record_budget": RECORD_BUDGET, "canonical_evidence_set_count": len(evidence_sets),
        "authority_grant_count": len(authority_grants), "h5_g3_lineage_sha256": plane_e["h5_g3_lineage_sha256"],
        "correct_position_balance": {"first": sum(1 for unit in evaluator_units if unit["correct_position"] == 0),
                                     "second": sum(1 for unit in evaluator_units if unit["correct_position"] == 1)},
        "family_counts": {family: sum(1 for unit in units if unit["family"] == family) for family in FAMILIES},
    }
...
def main() -> int:
    ...
    e_path, k_path, m_path = args.output_dir / "plane_e" / "evidence.json", args.output_dir / "plane_k" / "evaluator.json", args.output_dir / "meta" / "fixture-manifest.json"
    ...
    print(json.dumps({"plane_e_sha256": manifest["plane_e_sha256"], "plane_k_sha256": manifest["plane_k_sha256"],
                      "fixture_manifest_sha256": hashlib.sha256(m_bytes).hexdigest(), "h5_g3_lineage_sha256": manifest["h5_g3_lineage_sha256"],
                      "organization_cell_count": manifest["organization_cell_count"], "logical_model_call_count": manifest["logical_model_call_count"]}, sort_keys=True))
    return 0
