#!/usr/bin/env python3
"""Run one frozen D2-C2 confirmatory provider shard without classifying it."""
...
r2.CONCURRENCY = LOCAL_CONCURRENCY
...
def case_bundle(pair_index: int) -> dict[str, Any]:
    ...
    source_cases = generate_balanced_cases(
        ...
        prefix=f"d2c2-source-p{pair_index:03d}",
        policy=policy,
    )
    ...
    destination_cases = generate_balanced_cases(
        ...
        prefix=f"d2c2-destination-p{pair_index:03d}",
        policy=policy,
    ...
    eval_cases = generate_balanced_cases(
        rng_seed=eval_seed,
        count=EVAL_COUNT,
        prefix=f"d2c2-eval-p{pair_index:03d}",
        policy=policy,
...
def verify_cohort_lock(path: Path) -> dict[str, Any]:
    ...
    if lock.get("schema") != "d2-c2-confirmatory-cohort-lock-v0.1":
        raise AssertionError("unexpected C2 cohort-lock schema")
    if lock["cohort_pairs_sha256"] != EXPECTED_COHORT_SHA256:
        raise AssertionError("committed C2 cohort hash constant mismatch")
    if lock["seed_series"]["base"] != SEED_BASE:
        raise AssertionError("C2 seed-base mismatch")
    ...
    if computed != EXPECTED_COHORT_SHA256:
        raise AssertionError(
            f"C2 cohort lock mismatch: {computed} != {EXPECTED_COHORT_SHA256}"
        )
    if any(
        ...
        raise AssertionError("C2 cohort overlap integrity failure")
    return {
        "schema": lock["schema"],
...
def load_shard_range(shard_id: int, path: Path) -> tuple[int, int]:
    ...
    if data.get("provider_local_concurrency_per_shard") != LOCAL_CONCURRENCY:
        raise AssertionError("C2 local concurrency drift")
    ...
    for row in shards:
        ...
        if end < start:
            raise AssertionError("invalid C2 shard range")
        expected_indices.extend(range(start, end + 1))
        if sid == shard_id:
            selected = (start, end)
    if expected_indices != list(range(PAIR_COUNT)):
        raise AssertionError("C2 shard map does not cover 0..359 exactly once")
    if selected is None:
        raise AssertionError(f"unknown C2 shard id: {shard_id}")
    return selected


class Client(r2.Client):
    """R2 transport with C2 request-id provenance and counters."""

    def __init__(self, key: str) -> None:
        super().__init__(key)
        self.rng = random.Random(2026083001)
    ...
    def _request_id(self, phase: str, attempt: int) -> str:
        ...
        return f"d2c2-{safe}-{attempt}-{nonce:016x}"
...
def build_artifact(pair_index: int, source_public_score: float) -> dict[str, Any]:
    return make_artifact(
        artifact_id=f"d2-c2-confirmatory-capability-{pair_index:03d}",
...
def run_pair(client: Client, pair_index: int) -> dict[str, Any]:
    ...
    source_agent_id = (
        f"source-agent-d2c2-p{pair_index:03d}-{bundle['source_seed']}"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        default="output/d2-c2-confirmatory-provider-shard",
    )
    parser.add_argument(
        "--cohort-lock",
    ...
    output_path = output_dir / (
        f"d2-c2-confirmatory-provider-shard-{args.shard_id:02d}.json"
    )
    output_path.write_bytes(canonical_bytes(output))
    manifest = {
        "schema": "d2-c2-confirmatory-provider-shard-manifest-v0.1",
        "shard_id": args.shard_id,
        "start_pair": start_pair,
        "end_pair": end_pair,
    ...
    manifest_path = output_dir / (
        f"d2-c2-provider-shard-{args.shard_id:02d}-manifest.json"
    )
