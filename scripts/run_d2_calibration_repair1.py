    ...
    return {
        ...
        "public_case_batch_sha256": payload_sha(
    ...
    return {
        ...
        "integrity": {
            ...
            "development_evaluation_overlap_total": sum(
                pair["evaluation_development_overlap_count"]
                for pair in pair_records
            ),
            ...
            "all_fresh_logical_calls_three": all(
                pair["arms"]["fresh"]["logical_calls"] == 3
                for pair in pair_records
            ),
        ...
        "call_accounting": {
            ...
            "retry_frequency": (
                (total_physical - total_logical) / total_physical
            ),
            "retry_reasons": retry_reasons,
            "extra_key_total": extra_key_total,
            "strategy_omission_total": strategy_omission_total,
...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENCY) as pool:
        ...
        for future in as_completed(futures):
            ...
            print(
                f"D2_CALIBRATION_R1_PROGRESS {len(completed)}/{PAIR_COUNT} "
                f"pair={index}",
                flush=True,
            )
