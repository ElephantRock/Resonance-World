"""Fail-closed transport instrumentation for D2 terminal-response observability."""
...
class TransportLedger:
    ...
    def __init__(self) -> None:
        ...
        self._rows: dict[int, list[dict[str, Any]]] = {
            index: [] for index in range(MAX_LOGICAL_CALLS)
        }
...
def verified_origin_logical_index(
    ...
    """Verify independent HTTP origin against propagated logical-call authority."""
...
def transport_guard(
    ...
    """Guard physical sends and restore HTTP hooks only after provider workers drain."""
