...
def _failed_row(index: int) -> dict[str, object]:
    ...
    return {"status": "failed", "pair_index": index, "schema_id": schema_id, "schema_pair_index": local, "pair_public_id": f"d2c-{schema_id}-pair-{local:03d}", "failure_class": "synthetic_test_failure"}
...
def _complete_pair(index: int) -> dict[str, object]:
    ...
    def arm(actions: list[str], calls: int) -> dict[str, object]:
        return {
            ...
            "runner_final_score": sum(a == b for a, b in zip(actions, truth, strict=True)) / len(truth),
    ...
    return {
        "status": "complete", "pair_index": index, "schema_id": schema_id, "schema_pair_index": local,
        "pair_public_id": f"d2c-{schema_id}-pair-{local:03d}",
        "pair_lock_record": runner.pair_lock_record(index), "artifact": {}, "artifact_audit": {"passed": True},
        "evaluation_case_ids": [case["case_id"] for case in bundle["eval_cases"]], "evaluation_truth": truth,
        "arms": {"fresh": arm(wrong, 4), "description_only": arm(wrong, 9), "reproduced": arm(truth, 9), "source_developed": arm(truth, 9)},
    }
...
def _provider(complete_by_schema: dict[str, int] | None = None) -> dict[str, Any]:
    ...
    for index in range(540):
        schema_id, local = core.schema_and_local_index(index)
        rows.append(_complete_pair(index) if local < complete_by_schema[schema_id] else _failed_row(index))
    complete = sum(row["status"] == "complete" for row in rows)
    return {
        "schema": "d2c-schema-generalization-provider-output-v0.1", "status": "provider_campaign_complete_unclassified",
        "classification": None, "model": "glm-5-turbo", "temperature": 0.8,
        "attempted_pairs": 540, "complete_pairs": complete, "failed_pairs": 540 - complete,
        "schema_counts": {},
        "cohort_lock": {"schema": "d2c-schema-generalization-cohort-lock-v0.1", "pair_count": 540, "pairs_per_schema": 180, "cohort_pairs_sha256": runner.EXPECTED_COHORT_SHA256, "all_source_destination_overlaps_zero": True, "all_development_evaluation_overlaps_zero": True},
        "pair_records": rows, "aggregation_integrity": {"passed": True, "defects": [], "valid_shards": list(range(27)), "missing_shards": [], "invalid_shards": []},
        "transport_accounting": {}, **_hashes(), "production_historical_substrate_enabled": False,
    }


def _evaluate(path: Path) -> dict[str, Any]:
    return evaluator.evaluate(path, plan_path=PLAN, request_plan_path=REQUEST, schema_suite_path=SCHEMA_SUITE, sample_size_path=SAMPLE, cohort_lock_path=COHORT, shard_map_path=SHARDS)


def test_cohort_hash_and_runner_contract_are_exact():
    summary = runner.verify_cohort_lock(COHORT)
    assert summary["cohort_pairs_sha256"] == "559a4420a1d592d85fa350d087a8d4b945f4bf882a683c660a77cf9fdb6b9c04"
...
def test_per_schema_stats_pass_and_fail(monkeypatch):
    ...
    assert stats.evaluate_schema_scores([0.2] * 164, [0.2] * 164, [0.6] * 164, [0.6] * 164)["minimum_n_pass"] is False


def test_program_classifier_requires_all_three_schemas():
    base = {"minimum_n_pass": True, "P0": {"gate_pass": True}, "P1": {"gate_pass": True}, "P2": {"gate_pass": True}}
    all_pass = {schema_id: dict(base) for schema_id in core.SCHEMA_ORDER}
    assert evaluator._classify(all_pass, True)[0] == "D2c-S4"
    p0 = {k: dict(v) for k, v in all_pass.items()}; p0["parity_pair"] = {**base, "P0": {"gate_pass": False}}
    assert evaluator._classify(p0, True)[0] == "D2c-S1"
    p1 = {k: dict(v) for k, v in all_pass.items()}; p1["interval_pair"] = {**base, "P1": {"gate_pass": False}}
    assert evaluator._classify(p1, True)[0] == "D2c-S2"
    p2 = {k: dict(v) for k, v in all_pass.items()}; p2["pairwise_order"] = {**base, "P2": {"gate_pass": False}}
...
def test_evaluator_s4_with_165_analyzable_each(monkeypatch, tmp_path):
    ...
    path = tmp_path / "provider.json"; path.write_text(json.dumps(_provider(), sort_keys=True))
    result = _evaluate(path)
    assert result["classification"] == "D2c-S4"
    assert result["analyzable_pairs_by_schema"] == {schema_id: 165 for schema_id in core.SCHEMA_ORDER}
...
def test_one_schema_below_minimum_forces_s0(monkeypatch, tmp_path):
    ...
    path = tmp_path / "provider.json"; path.write_text(json.dumps(_provider({"parity_pair": 164, "interval_pair": 165, "pairwise_order": 165}), sort_keys=True))
    result = _evaluate(path)
...
def test_missing_full_shard_leaves_schema_max_160(tmp_path):
    ...
    output = aggregator.aggregate(tmp_path, plan_path=PLAN, request_plan_path=REQUEST, schema_suite_path=SCHEMA_SUITE, sample_size_path=SAMPLE, cohort_lock_path=COHORT, shard_map_path=SHARDS)
